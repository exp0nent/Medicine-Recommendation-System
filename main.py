...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        # Convert spaces back to underscores to match symptoms_dict keys
        symptom_key = item.strip().replace(' ', '_')

        # Check if this symptom exists in our dictionary
...
def home():
    if request.method == 'POST':
        ...
        if not symptoms or symptoms.lower() == "symptoms":
            ...
            return render_template('index.html', message=message, entered_symptoms=symptoms)
        else:
            ...
            return render_template('index.html', predicted_disease=predicted_disease, dis_des=dis_des,
                                   ...
                                   workout=workout, entered_symptoms=symptoms)
...
# New API for symptoms auto-suggest (simple feature)
...
def suggest():
    ...
    # Get the last entered symptom (after last comma)
    last_symptom = query.split(',')[-1].strip()

    if not last_symptom:
        return jsonify([])

    suggestions = [sym for sym in symptoms_dict.keys() if last_symptom in sym.lower()]
    return jsonify(suggestions[:8])  # return top 8 matches
